...
class WhoopIncrementalStore {
  ...
  async fetchIncrementalData(type, accessToken) {
    ...
    let startDate = null;
    if (latestDate) {
      // Start from just after the latest record to avoid duplicates
      // Add a small time buffer (1 minute) instead of a full day
      const latestTime = new Date(latestDate);
      latestTime.setMinutes(latestTime.getMinutes() + 1);
      
      // Don't fetch if the start time would be in the future
      const now = new Date();
      if (latestTime <= now) {
        startDate = latestTime.toISOString();
        console.log(`📅 Existing data found. Fetching from: ${startDate}`);
      } else {
        console.log(`📅 Latest data is already current. No new records to fetch.`);
        return {
          records: [],
          lastUpdate: new Date().toISOString(),
          totalCount: 0,
          date_range: { earliest: latestDate, latest: latestDate },
          newRecordsFetched: 0,
