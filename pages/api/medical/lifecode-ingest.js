...
function extractDates(ptText){
  ...
  const good=["RECEBIDO/COLETADO EM","COLETADO EM","RECEBIDO EM","DATA DA FICHA","COLETA","COLETADO:"];
  const bad=["LIBERADO EM","EXAME LIBERADO EM","IMPRESSO EM"];
  let collected=null, reported=null;
  for(const line of text.split(/\r?\n/)){
    const L=line.toUpperCase(); const dates=[...line.matchAll(ddmmyyyy)].map(m=>`${m[3]}-${m[2]}-${m[1]}`);
    if(!dates.length) continue;
    if(good.some(h=>L.includes(h))) collected=collected||dates[0];
    else if(bad.some(h=>L.includes(h))) reported=reported||dates[0];
  }
  if(!collected){ const m=text.match(ddmmyyyy); if(m){ const x=m[0].match(/([0-3]\d)\/([01]\d)\/(\d{4})/); if(x) collected=`${x[3]}-${x[2]}-${x[1]}`; } }
...
export default async function handler(req,res){
  ...
  try{
    ...
    form.parse(req, async (err,fields,files)=>{
      ...
      if(!f?.filepath) return res.status(400).json({error:"No file uploaded"});
      ...
      const sys = [
        "Extract key findings from this Lifecode report.",
        "Return ONLY a JSON array of objects (no markdown).",
        "Each object:",
        "- collected_at (YYYY-MM-DD) if known, else null",
        "- reported_at (YYYY-MM-DD) if known, else null",
        "- section (string or null)",
        "- item (original name from report; keep language as-is; keep casing as-is)",
      ...
      if(rows.length){
        const {error}=await supabase.from("lifecode_items").insert(rows);
